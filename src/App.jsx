...
function exportMenuPDF(menu){
  ...
  w.document.write(`<!DOCTYPE html><html dir="rtl" lang="he"><head><meta charset="UTF-8"><link href="https://fonts.googleapis.com/css2?family=DM+Sans:ital,opsz,wght@0,9..40,300;0,9..40,400;0,9..40,600;0,9..40,700&family=DM+Serif+Display&display=swap" rel="stylesheet"><title>${menu.name}</title><style>*{box-sizing:border-box;margin:0;padding:0;}body{font-family:'DM Sans',system-ui,sans-serif;color:#1c1917;background:#fff;padding:20px 64px 56px;direction:rtl;text-align:center;}.menu-title-area{margin-bottom:48px;padding-bottom:32px;border-bottom:1px solid #e8e4df;}.menu-title{font-family:'DM Serif Display',serif;font-size:44px;font-weight:400;color:#1e3a5f;letter-spacing:-0.5px;line-height:1.2;}.section{margin-bottom:36px;}.section-title{font-size:13px;font-weight:700;color:#888;letter-spacing:2px;text-transform:uppercase;margin-bottom:16px;padding-bottom:10px;border-bottom:1px solid #eeebe6;text-align:center;}.dish{padding:12px 0;border-bottom:1px solid #f4f2ef;font-size:18px;font-weight:400;color:#1c1917;line-height:1.5;text-align:center;}.dish:last-child{border-bottom:none;}.notes-section{margin-top:40px;}.notes-content{font-size:14px;color:#444;line-height:1.8;text-align:center;}@media print{body{padding:20px 48px 40px;}}</style></head><body><div class="menu-title-area"><div class="menu-title">${menu.name}</div></div>${sections.map(sec=>`<div class="section"><div class="section-title">${sec.title||""}</div>${(sec.dishes||[]).filter(d=>d.trim()).map(d=>`<div class="dish">${d}</div>`).join("")}</div>`).join("")}${hasNotes?`<div class="notes-section"><div class="section-title">תוספות</div><div class="notes-content">${menu.notes}</div></div>`:""}<script>window.onload=()=>{window.print();}<\/script></body></html>`);
  w.document.close();
}
