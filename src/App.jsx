                      ...
                      <div style={{background:T.navyLight,border:`1px solid ${T.navyBorder}`,borderRadius:12,padding:14,marginTop:8}}>
                        ...
                        <div style={{display:"flex",flexDirection:"column",gap:8}}>
                          <div style={{display:"flex",gap:8}}>
                            ...
                            <div style={{flex:1}}><div style={{fontSize:10,color:T.textMid,fontWeight:600,marginBottom:3}}>שער קנייה</div><Inp type="number" value={editPurch.purchase.price} onChange={e=>setEditPurch(ep=>({...ep,purchase:{...ep.purchase,price:e.target.value}}))}/></div>
                          </div>
                          <div style={{display:"flex",gap:8}}>
                          ...
                          <div style={{background:T.dangerBg,border:`1px solid ${T.dangerBorder}`,borderRadius:12,padding:14,marginTop:8}}>
                            ...
                            <div style={{display:"flex",flexDirection:"column",gap:8}}>
                              <div style={{display:"flex",gap:8}}>
                                ...
                                <div style={{flex:1}}><div style={{fontSize:10,color:T.textMid,fontWeight:600,marginBottom:3}}>שער מכירה</div><Inp type="number" value={editSale.sale.price} onChange={e=>setEditSale(es=>({...es,sale:{...es.sale,price:e.target.value}}))}/></div>
                              </div>
                              <div style={{display:"flex",gap:8}}>
                                <div style={{flex:1}}><div style={{fontSize:10,color:T.textMid,fontWeight:600,marginBottom:3}}>עמלה</div><Inp type="number" value={editSale.sale.commission||0} onChange={e=>setEditSale(es=>({...es,sale:{...es.sale,commission:e.target.value}}))}/></div>
                                <div style={{flex:1}}><div style={{fontSize:10,color:T.textMid,fontWeight:600,marginBottom:3}}>מס (%)</div><Inp type="number" value={editSale.sale.taxRate||25} onChange={e=>setEditSale(es=>({...es,sale:{...es.sale,taxRate:e.target.value}}))}/></div>
                              </div>
                              <div style={{display:"flex",gap:8}}>
                      ...
                      {editDiv?.assetId===a.id&&(
                        <div style={{background:T.successBg,border:"1px solid #bbf7d0",borderRadius:12,padding:14,marginBottom:8}}>
                          ...
                          <div style={{display:"flex",flexDirection:"column",gap:8}}>
                            ...
                            <div style={{display:"flex",gap:8}}>
                              {a.currency!=="ILS"&&<div style={{flex:1}}><div style={{fontSize:10,color:T.textMid,fontWeight:600,marginBottom:3}}>שער המרה</div><Inp type="number" value={editDiv.dividend.rateUsed} onChange={e=>setEditDiv(ed=>({...ed,dividend:{...ed.dividend,rateUsed:e.target.value}}))}/></div>}
                            ...
                            <div><div style={{fontSize:10,color:T.textMid,fontWeight:600,marginBottom:3}}>הערה</div><Inp value={editDiv.dividend.notes||""} onChange={e=>setEditDiv(ed=>({...ed,dividend:{...ed.dividend,notes:e.target.value}}))}/></div>
                            <div style={{display:"flex",gap:8}}>
...
function exportMenuPDF(menu){
  ...
  w.document.write(`<!DOCTYPE html><html dir="rtl" lang="he"><head><meta charset="UTF-8"><link href="https://fonts.googleapis.com/css2?family=Playfair+Display:ital,wght@0,400;0,500;1,400&family=Assistant:wght@300;400;600&display=swap" rel="stylesheet"><title>${menu.name}</title><style>*{box-sizing:border-box;margin:0;padding:0;}body{font-family:'Assistant',sans-serif;color:#1c1917;background:#fff;padding:72px 80px;direction:rtl;text-align:center;}.frame{border:1px solid #c8c2b8;padding:56px 64px;min-height:calc(100vh - 144px);}.title{font-family:'Playfair Display',serif;font-size:65px;font-weight:400;font-style:italic;color:#1c1917;letter-spacing:1px;margin-bottom:48px;}.section{margin-bottom:40px;}.section-title{font-family:'Assistant',sans-serif;font-size:35px;font-weight:600;letter-spacing:4px;text-transform:uppercase;color:#a8a29e;margin-bottom:20px;}.section-title::before,.notes-title::before{content:"— ";}.section-title::after,.notes-title::after{content:" —";}.dish{font-family:'Playfair Display',serif;font-size:30px;font-weight:400;color:#1c1917;line-height:1.6;margin-bottom:4px;}.section-divider{width:500px;height:1px;background:#c8c2b8;margin:40px auto;}.notes-title{font-family:'Assistant',sans-serif;font-size:35px;font-weight:600;letter-spacing:4px;text-transform:uppercase;color:#a8a29e;margin-bottom:16px;}.notes-content{font-family:'Assistant',sans-serif;font-size:20px;font-weight:300;color:#57534e;line-height:2;}@media print{body{padding:48px 56px;}}</style></head><body><div class="frame"><div class="title">${menu.name}</div>${sections.length>0?sections.map((sec,i)=>`<div class="section"><div class="section-title">${(sec.title||"").replace(/^מנות\s*/,"")}</div>${(sec.dishes||[]).filter(d=>d.trim()).map(d=>`<div class="dish">${d}</div>`).join("")}</div>${i<sections.length-1?'<div class="section-divider"></div>':""}`).join(""):""}${menu.notes?`<div class="section-divider"></div><div class="notes-title">תוספות</div><div class="notes-content">${menu.notes}</div>`:""}</div><script>window.onload=()=>{window.print();}<\/script></body></html>`);
  w.document.close();
}
