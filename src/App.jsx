...
הערות התפריט: ${(item.notes||"").replace(/<[^>]+>/g,"")||"אין"}`;
      ...
      const resp=await fetch("/api/anthropic",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({
        max_tokens:6000,
        system:`אתה שף ישראלי מקצועי שמכין רשימת קניות מפורטת.

דרך חשיבה נדרשת:
1. קרא את כל המנות בתפריט
2. לכל מנה — חשוב מה המרכיבים האמיתיים להכנתה
3. אחד מרכיבים שחוזרים במנות שונות לשורה אחת עם הכמות המצטברת
4. חשב כמויות ריאליות למספר הסועדים

דוגמאות חישוב למרכיבים לפי מנה (ל-8 סועדים):
- "קבב" = בשר טחון 1.5 ק"ג, בצל 2 יח', פטרוזיליה 1 צרור
- "פרגית" = חזה עוף 2 ק"ג, שום 5 שיני
- "שיפוד כנפיים" = כנפי עוף 2 ק"ג
- "שיפוד לבבות" = לבבות עוף 1 ק"ג
- "שיפוד מרגז" = מרגז 1 ק"ג
- "סלט ירקות קצוץ" = עגבניות 1 ק"ג, מלפפונים 800 גרם, פלפל 3 יח', בצל 1 יח'
- "סלט עגבניות שום פלפל חריף" = עגבניות 800 גרם, שום 3 שיני, פלפל חריף 2 יח'
- "סלט קולסלאו" = כרוב לבן 1 יח', גזר 3 יח', מיונז 200 גרם
- "סלט ביצים" = ביצים 10 יח', מיונז 100 גרם, בצל ירוק 1 צרור
- "חומוס גרגרים" = גרגרי חומוס מבושלים 500 גרם, טחינה 200 גרם, לימון 2 יח'
