...
function renderMarkdown(text){
  ...
  html=html.replace(/^### (.+)$/gm,"<div style='font-weight:700;font-size:13px;color:#1e3a5f;margin:10px 0 4px'>$1</div>");
  html=html.replace(/^## (.+)$/gm,"<div style='font-weight:700;font-size:14px;color:#1e3a5f;margin:12px 0 6px'>$1</div>");
  html=html.replace(/^# (.+)$/gm,"<div style='font-weight:700;font-size:15px;color:#1e3a5f;margin:14px 0 6px'>$1</div>");
  html=html.replace(/^[-•\*] (.+)$/gm,"<div style='display:flex;gap:6px;margin:2px 0'><span style='color:#1e3a5f;flex-shrink:0'>•</span><span>$1</span></div>");
  html=html.replace(/^\d+\. (.+)$/gm,function(m,p1){return "<div style='display:flex;gap:6px;margin:2px 0'><span style='color:#1e3a5f;font-weight:600;flex-shrink:0'>·</span><span>"+p1+"</span></div>";});
  ...
  for(var i=0;i<lines.length;i++){
    ...
    if(line.includes("|")){
      ...
      if(inTable){
        var tableHtml="<div style='overflow-x:auto;margin:8px 0'><table style='width:100%;border-collapse:collapse;font-size:12px'>";
        tableRows.forEach(function(row,ri){
          tableHtml+="<tr>";
          row.forEach(function(cell){
            var tag=ri===0?"th":"td";
            var style=ri===0
              ?"padding:6px 8px;background:#1e3a5f;color:#fff;font-weight:600;text-align:right"
              :"padding:6px 8px;border-bottom:1px solid #e6e2db;text-align:right";
      ...
      else{result.push("<span style='display:block'>"+line+"</span>");}
    }
  }
  if(inTable){
    var tableHtml="<div style='overflow-x:auto;margin:8px 0'><table style='width:100%;border-collapse:collapse;font-size:12px'>";
    tableRows.forEach(function(row,ri){
      ...
      row.forEach(function(cell){
        var tag=ri===0?"th":"td";
        var style=ri===0?"padding:6px 8px;background:#1e3a5f;color:#fff;font-weight:600;text-align:right":"padding:6px 8px;border-bottom:1px solid #e6e2db;text-align:right";
        tableHtml+="<"+tag+" style='"+style+"'>"+cell.trim()+"</"+tag+">";
      });
      tableHtml+="</tr>";
    });
...
- summary חייב להיות טקסט נקי בעברית בלבד ללא סימנים מיוחדים`;
  ...
  const deleteConversation=async(id)=>{
    ...
    setSavedConversations(p=>p.filter(c=>c.id!==id));
    if(currentConvId===id){setCurrentConvId(null);setAgentHistory([]);}
  ...
  const runAgent = async (overrideQuery) => {
    ...
    // טען חדשות אם לא נטענו עדיין
    if(newsItems.length===0){
      await fetchNews().catch(()=>{});
    }

    const totalPortfolioILS = activeAssets.reduce((s,a) => s + currentValILS(a), 0);
    const portfolioLines = activeAssets.map(a => {
      ...
      const pnlPct = avg && price ? (((price-avg)/avg)*100).toFixed(2) : "?";
      const snapPct = price && snapshot ? (((price-snapshot)/snapshot)*100).toFixed(2) : null;
      const weight = totalPortfolioILS ? ((valILS/totalPortfolioILS)*100).toFixed(2) : "?";
      const divTotal = totalDividendsILS(a);
      return `${a.security} [${ticker}]:
  - יחידות: ${totalShares(a).toFixed(4)} | שער קנייה ממוצע: ${fmtForeign(avg,a.currency)}
  - מחיר נוכחי: ${price ? fmtForeign(price,a.currency) : "לא ידוע"}${snapPct!==null?` (${snapPct>=0?"+":""}${snapPct}% מאתמול)`:""}
  - שווי: ${fmt(valILS)} (${weight}% מהתיק)
  - P&L לא ממומש: ${pnl>=0?"+":""}${fmt(pnl)} (${pnlPct}%)
  - דיבידנדים כולל: ${fmt(divTotal)}`;
    }).join("\n\n");

    const realizedLines = soldAssets.length > 0
      ? `\nניירות שנמכרו: ${soldAssets.map(a=>`${a.security}: P&L ממומש ${fmt(realizedPnLILS(a))}`).join(", ")}`
      : "";
...
P&L שוטף: ${totalPnL>=0?"+":""}${fmt(totalPnL)} (${totalPnLPct.toFixed(2)}%)
P&L ממומש: ${fmt(totalRealized)}
דיבידנדים כולל: ${fmt(allDividendsTotal)}

פירוט ניירות:
${portfolioLines||"תיק ריק"}
${realizedLines}

חדשות שוק עדכניות:
${newsContext}`;

    const conversationHistory = agentHistory.slice(-6).flatMap(h => [
...
{tab==="agent"&&(
  <div style={{display:"flex",flexDirection:"column",gap:12}}>
    <div style={{display:"flex",background:T.bg,border:`1px solid ${T.border}`,borderRadius:12,gap:3,marginBottom:8}}>
      {[["chat","שיחה חדשה"],["history","היסטוריית שיחות"]].map(([v,l])=>(
        <button key={v} onClick={()=>{setAgentView(v);if(v==="history")loadConversations();}}
          style={{flex:1,padding:"7px",borderRadius:9,fontFamily:T.font,fontSize:12,
            fontWeight:600,cursor:"pointer",border:"none",
            background:agentView===v?T.surface:"transparent",
            color:agentView===v?T.navy:T.textSub,
            boxShadow:agentView===v?"0 1px 4px rgba(0,0,0,.08)":"none",
            transition:"all .15s"}}>{l}</button>
      ))}
    </div>
    ...
    {agentView==="history"&&(
      <div style={{display:"flex",flexDirection:"column",gap:10}}>
        {confirmDeleteConv&&<ConfirmModal
          message="למחוק שיחה זו לצמיתות?"
        ...
        {savedConversations
          .filter(c=>!historySearch||c.title.includes(historySearch))
          .map(c=>(
            <Card key={c.id} style={{padding:14}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:8}}>
                <div style={{flex:1,cursor:"pointer"}} onClick={async()=>{
                  const {data}=await supabase.from('agent_conversations')
                    .select('messages').eq('id',c.id).single();
                  if(data?.messages){
                  ...
                  <div style={{fontSize:13,fontWeight:600,color:T.text,marginBottom:3}}>{c.title}</div>
                  <div style={{fontSize:11,color:T.textSub,display:"flex",gap:8}}>
                    {c.who&&<span>{c.who==="ס"?"ספיר":"אדיר"}</span>} · <span>{new Date(c.updated_at).toLocaleDateString("he-IL")}</span>
                  </div>
                </div>
                <button onClick={()=>setConfirmDeleteConv(c.id)}
                  style={{background:T.dangerBg,border:`1px solid ${T.dangerBorder}`,
                    borderRadius:8,padding:"5px 8px",cursor:"pointer",flexShrink:0}}>
        ...
        {savedConversations.filter(c=>!historySearch||c.title.includes(historySearch)).length===0&&(
          <div style={{textAlign:"center",color:T.textSub,padding:32,fontSize:13}}>
            {historySearch?"לא נמצאו שיחות":"אין שיחות שמורות עדיין"}
          </div>
        )}
      </div>
    )}
      ...
      <Card style={{background:`linear-gradient(135deg,#1e3a5f 0%,#2d5282 100%)`,border:"none",padding:18,paddingBottom:160}}>
        <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:12}}>
          ...
          {agentHistory.length>0&&(
            <button onClick={()=>{setAgentHistory([]);setCurrentConvId(null);}}
        ...
        {agentHistory.map(h=>(
          <div key={h.id} style={{marginTop:8}}>
            <div style={{display:"flex",justifyContent:"flex-start",marginBottom:6}}>
              <div style={{maxWidth:"78%",background:"rgba(255,255,255,.15)",border:"1px solid rgba(255,255,255,.2)",borderRadius:"14px 14px 14px 2px",padding:"10px 14px",fontSize:13,color:"#fff",fontWeight:600,direction:"rtl",lineHeight:1.5}}>
                {h.q}
              </div>
            </div>
            <div style={{display:"flex",justifyContent:"flex-end"}}>
              <div style={{maxWidth:"85%",background:"rgba(255,255,255,.92)",border:"none",borderRadius:"14px 14px 2px 14px",padding:"12px 14px",fontSize:13,color:T.text,lineHeight:1.8,direction:"rtl"}}
        ...
        {agentLoading&&(
          <div style={{display:"flex",justifyContent:"flex-end",marginTop:8}}>
            <div style={{background:"rgba(255,255,255,.92)",borderRadius:"14px 14px 2px 14px",padding:"14px 18px",display:"flex",alignItems:"center",gap:5}}>
              {[0,1,2].map(i=>(
...
const exportRecipePDF=(recipe)=>{
  ...
  if(!w)return;
  const cats=Array.isArray(recipe.categories)?recipe.categories:recipe.category?[recipe.category]:[];
  w.document.write(`<!DOCTYPE html><html dir="rtl" lang="he">
<head><meta charset="UTF-8"/><title>${recipe.name}</title>
<style>
  body{font-family:system-ui,sans-serif;direction:rtl;padding:32px;color:#1c1917;max-width:680px;margin:0 auto;}
  h1{font-size:26px;font-weight:700;margin-bottom:4px;}
  .meta{font-size:13px;color:#78716c;margin-bottom:24px;display:flex;gap:16px;flex-wrap:wrap;}
  .section-title{font-size:14px;font-weight:700;color:#1e3a5f;margin:20px 0 10px;padding-bottom:6px;border-bottom:2px solid #e6e2db;}
  .ingredient-row{display:flex;gap:8px;padding:6px 0;border-bottom:1px solid #f0ede8;font-size:13px;}
  .ingredient-row .qty{color:#78716c;min-width:60px;}
  ...
  <h1>${recipe.name}</h1>
  <div class="meta">
    ${cats.length?`<span>${cats.join(", ")}</span>`:""}
    ${recipe.servings?`<span>${recipe.servings} אנשים</span>`:""}
  </div>
  ${(recipe.ingredients||[]).filter(i=>i.item).length?`
    <div class="section-title">מצרכים</div>
    ${(recipe.ingredients||[]).filter(i=>i.item).map(i=>`
      <div class="ingredient-row">
        <span class="qty">${i.qty||""} ${i.unit||""}</span>
        <span>${i.item}</span>
      </div>`).join("")}`:""}
  ${(recipe.steps||[]).filter(s=>s.replace(/<[^>]+>/g,"").trim()).length?`
    <div class="section-title">אופן הכנה</div>
    ${(recipe.steps||[]).filter(s=>s.replace(/<[^>]+>/g,"").trim()).map((s,i)=>`
      <div class="step-row">
        <div class="step-num">${i+1}</div>
        <div class="step-text">${s}</div>
      </div>`).join("")}`:""}
  ${recipe.prepNotes||recipe.notes?`
    <div class="section-title">הכנות מקדימות</div>
    <div class="notes">${recipe.prepNotes||recipe.notes}</div>`:""}
...
הערות התפריט: ${(item.notes||"").replace(/<[^>]+>/g,"")||"אין"}`;
      ...
      const resp=await fetch("/api/anthropic",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({
        max_tokens:6000,
        system:`אתה שף מקצועי שיוצר רשימות קניות ממתכונים ותפריטים.

המשימה: לקרוא את המנות ואת ההערות, ולגזור מהן את כל מצרכי הקנייה הנדרשים בכמויות ריאליות למספר הסועדים.

חוקי יחידות (3 יחידות בלבד — ק"ג, ליטר, יח'):

ק"ג — לכל מה שנמכר במשקל:
- בשר מכל סוג (בקר, עוף, הודו, טלה, דגים)
- מוצרי בשר מעובדים (קבב, המבורגר, נקניקיות, שיפודים, מרגז)
- ירקות עליים ומוצרי גרגירים (חסה, תרד, אורז, גרגרי חומוס)
- אבקות (קמח, סוכר, אורז, קוסקוס)

ליטר — לנוזלים:
- שמנים, מים, חלב, משקאות, רטבים נוזליים

יח' — לפריטים שנספרים בודדים בחנות:
- ירקות/פירות שלמים (עגבנייה, מלפפון, בצל, לימון, תפוח)
- ביצים, לחמים, פיתות, טורטיות
- קופסאות/שקיות/צנצנות (טונה, שעועית, טחינה)
- יח' חייב להיות מספר שלם בלבד, לא עשרוני

עקרון זיהוי: אם המוצר הוא בשר או דג בצורה כלשהי — תמיד ק"ג. אם נוזל — ליטר. אם פריט בודד — יח' שלם.

איסורים:
- אסור לכלול תבלינים (מלח, פלפל, כמון, פפריקה וכו')
- אסור לכלול שמנים בסיסיים (שמן זית, שמן קנולה) — אלא אם נדרשים בכמות משמעותית כמרכיב מרכזי
- אסור להשאיר qty ריק או 0

חובות:
- לכלול את כל הפריטים שמוזכרים בהערות/תוספות התפריט (גם אם לא במבנה של מנה)
- לאחד מוצרים זהים שמופיעים במנות שונות לשורה אחת עם סכום הכמויות
- להתחשב במספר הסועדים בחישוב הכמויות
- לכלול מצרכי בסיס הנדרשים להכנה (למשל קמח למאפים, ביצים, שמרים)

פורמט JSON בלבד ללא טקסט נוסף:
{"items":[{"name":"שם המוצר","qty":"מספר","unit":"ק\\"ג/ליטר/יח'"}]}`,
  ...
  const save=async()=>{
    ...
    const dbItem={id:saved.id,type:saved.type,name:saved.name,categories:saved.categories,servings:saved.servings,ingredients:saved.ingredients||[],steps:saved.steps||[],sections:saved.sections||[],notes:saved.notes||'',prep_notes:saved.prepNotes||'',concepts:saved.concepts||[]};
    if(editId){
                ...
                {mode==="recipe"&&(<>
                  ...
                  {form.steps.map((st,i)=>(
                    <div key={i} style={{display:"flex",gap:6,alignItems:"flex-start"}}>
                      <div style={{width:22,height:22,borderRadius:"50%",background:T.navy,color:"#fff",display:"flex",alignItems:"center",justifyContent:"center",fontSize:11,fontWeight:700,flexShrink:0,marginTop:8}}>{i+1}</div>
                      <div style={{flex:1}}>
                        <RichTextEditor value={st} onChange={v=>setForm(f=>({...f,steps:f.steps.map((x,j)=>j===i?v:x)}))} placeholder={`שלב ${i+1}…`} minHeight={60}/>
                      </div>
                      {form.steps.length>1&&(
                        <button onClick={()=>setForm(f=>({...f,steps:f.steps.filter((_,j)=>j!==i)}))} style={{background:"none",border:"none",color:T.textSub,cursor:"pointer",fontSize:18,padding:"8px 4px",flexShrink:0}}>×</button>
                      )}
                    </div>
                  ))}
        ...
        <div>
          ...
          <Card>
            ...
            {sel.type==="recipe"&&(<>
              ...
              {sel.steps?.length>0&&<div style={{marginTop:14}}><div style={{fontSize:11,fontWeight:700,color:T.textMid,letterSpacing:.5,textTransform:"uppercase",marginBottom:10}}>אופן הכנה</div>{sel.steps.map((st,i)=><div key={i} style={{display:"flex",gap:10,marginBottom:10}}><div style={{width:24,height:24,borderRadius:"50%",background:T.navy,color:"#fff",display:"flex",alignItems:"center",justifyContent:"center",fontSize:11,fontWeight:700,flexShrink:0}}>{i+1}</div><div className="recipe-content" style={{fontSize:13,color:T.text,lineHeight:1.7}} dangerouslySetInnerHTML={{__html:st}}/></div>)}</div>}
    ...
    <div style={{padding:"0 0 40px"}}>
      ...
      <div style={{padding:16,display:"flex",flexDirection:"column",gap:14}}>
        {tab==="general"&&(<>
          ...
          <Card>
            <div style={{fontSize:13,fontWeight:700,color:T.navy,marginBottom:12}}>סגנונות ארוחות</div>
...
export default function App(){
  ...
  const [deviceAuthed,setDeviceAuthed]=useState(false);
  const [deviceChecking,setDeviceChecking]=useState(true);
  useEffect(()=>{
    const verify=async()=>{
      const deviceId=localStorage.getItem('device_id');
      const localAuth=localStorage.getItem('device_authorized');
      if(!deviceId||localAuth!=='1'){
  ...
  if(deviceChecking)return(
    <div style={{minHeight:"100vh",background:`linear-gradient(160deg,#0d1f35 0%,#1e3a5f 55%,#2d5282 100%)`,display:"flex",alignItems:"center",justifyContent:"center"}}>
      ...
      <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:20}}>
        ...
        <div style={{width:28,height:28,border:"2.5px solid rgba(255,255,255,.3)",borderTop:"2.5px solid #fff",borderRadius:"50%",animation:"spin 1s linear infinite"}}/>
      </div>
    </div>
  );
