...
הערות התפריט: ${(item.notes||"").replace(/<[^>]+>/g,"")||"אין"}`;
      ...
      const resp=await fetch("/api/anthropic",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({
        max_tokens:6000,
        system:`אתה שף מקצועי. צור רשימת קניות מהתפריט.

חוק יחידות — פשוט וברור:
1. בשר / עוף / דגים / כל נגזרת שלהם = ק"ג בלבד
2. נוזלים = ליטר בלבד
3. הכל אחר = יח' (מספר שלם בלבד)

חישוב כמויות לסועדים:
- עוף/בשר/דגים: 400-500 גרם לאדם = 0.4-0.5 ק"ג לאדם
- ירקות גדולים: 1 יח' לכל 2-3 אנשים
- ירקות קטנים: 1 יח' לאדם
- לחמים/מאפים: לפי הצורך
- ביצים: לפי הצורך
- נוזלים: לפי הצורך

כלול גם את פריטי ההערות/תוספות.
אל תכלול תבלינים ושמנים.
אחד מוצרים כפולים.

החזר JSON בלבד:
{"items":[{"name":"שם","qty":"מספר","unit":"ק\\"ג או ליטר או יח'"}]}`,
